...
## [2026-03-04] Netlify CText 代理 JSON 透传校验修复
0. Tags / 标签
   - ctext, infra
1. Time
   - 2026-03-04
2. 需求明确 / Goal
   - 修复 Netlify 函数返回 HTML 导致前端 `JSON.parse` 句法错误的问题，确保代理端只返回可解析 JSON。
3. 操作 / Actions
   - 更新 `netlify/functions/ctext-searchtexts.js`：
     - 上游请求候选顺序改为优先 `https://api.ctext.org/searchtexts`，其次 `func=searchtexts`；
     - 增加上游返回体 JSON 形态校验（必须以 `{` 或 `[` 开头）；
     - 若上游返回非 JSON 或 HTTP 错误，函数统一返回 `502 + JSON error`，不再透传 HTML 页面。
4. 解决 / Outcome
   - 代理函数输出稳定为 JSON，前端不再因 HTML 透传触发 JSON.parse 报错。
5. 复盘 / Retrospective
   - 代理层不能假设上游总是 JSON；应在函数端做“内容类型/形态兜底校验”，把错误收敛成统一 JSON 错误格式。
