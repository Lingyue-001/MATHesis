...
## [2026-03-12] GitHub Pages 静态缓存链路落地（cache-first + mapped-text 构建）
0. Tags / 标签
   - ctext, infra
1. Time
   - 2026-03-12
2. 需求明确 / Goal
   - 在不依赖线上动态代理的前提下，复用本地已稳定的 CText 检索结果，形成可上线的 GitHub Pages 静态检索基线。
3. 操作 / Actions
   - 前端（`src/transcriptions/tei_hanshu/1a.html`）新增 cache-first 逻辑：
     - 优先读取 `static/ctext-cache.json`；
     - 未命中时再走既有 middleware/json fallback；
     - `auto` 模式在 `*.github.io` 主机默认走 JSON fallback，避免无后端时空请求。
   - 新增构建脚本 `scripts/build-ctext-cache.mjs`：
     - 支持 `--terms` 手动补关键词；
     - 支持 `--mapped-text` 仅构建指定文本内实际可映射词；
     - 默认仅收集 CJK 词，避免英文词拖慢构建；
     - 增加逐词进度、请求超时、失败汇总，提升可诊断性。
   - 新增 `npm` 命令：
     - `build:ctext-cache`。
   - 新增并提交缓存文件：
     - `static/ctext-cache.json`（本轮实测为 10 entries，`failureCount=0`）。
   - 文档更新（`README.md`）：
     - 增加 GitHub Pages 模式使用说明；
     - 明确 `--terms` 手动补词与 `--mapped-text` 按文本筛词用法。
4. 解决 / Outcome
   - 线上静态页面可优先命中本地预构建缓存，规避“线上实时抓取不稳定”主痛点；
   - 缓存构建流程收敛为“先按 mapped text 小范围稳定运行，再增量扩展”。
5. 复盘 / Retrospective
   - “可上线稳定性”与“全量抓取完整性”应拆分：先保证点击到的词可稳定返回，再做增量扩展；
   - 缓存构建工具必须内置可观测性（逐词日志、超时、失败样本），否则排障成本会持续上升。
