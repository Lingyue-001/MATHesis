...
## [2026-03-04] CText 自有代理接入测试（test ctext api）
0. Tags / 标签
   - ctext, infra
1. Time
   - 2026-03-04
2. 需求明确 / Goal
   - 针对线上 `NetworkError` 与公共代理返回非 JSON 的失败场景，切换为“自有代理优先”的 JSON 检索链路，便于稳定测试与回滚。
3. 操作 / Actions
   - 在 `src/transcriptions/tei_hanshu/1a.html` 增加 `ctextProxy` 参数支持：
     - `ctextSource=json` 且传入 `ctextProxy=https://<proxy-domain>` 时，请求走 `<proxy>/searchtexts`；
     - 未提供 `ctextProxy` 时维持原有直连行为；
     - JSON 请求使用 `credentials: omit`。
   - 在 `src/js/debugFlags.mjs` 增加 `ctextProxy` 解析字段。
   - 新增 `CLOUDFLARE_WORKER_CTEXT_PROXY.js` 作为最小可部署代理模板。
   - 更新 `README.md`、`DEBUG_FLAGS_REFERENCE.md`、`DEBUG_FLAGS_QUICKSTART.md` 的使用说明。
4. 解决 / Outcome
